...
(function() {
  ...
  var tokens = ['{', '%', '!', '=', '#', '$', '_', '@', '!'];
  var tokenizer = new RegExp('{([' + tokens.join('') + '])|#import|#extends');
  var terminators = {};
  var token;
  for (var i = 0; i < tokens.length; i++) {
    token = tokens[i];
    terminators[token] = new RegExp('[' + token + ']}');
  }
  // Special cases
  terminators['{'] = new RegExp('}}'); // mustaches
  terminators['#extends'] = terminators['#import'] = new RegExp('\n');
  ...
  p.transform = function(token, input, index, tokenLength) {
    var end = terminators[token].exec(input);
    if (!end) {
  ...
  p.tokenHandlers = {
    ...
    '!': function component(body) {
      this.insertJavascriptCode(["var __uniqueId = _.uniqueId('__bg_component');",
        "emit(['<div id=\"' + __uniqueId + '\" />']);",
        "self.view._addChildFromTemplate(__uniqueId, " + body + ");"
      ].join(''));
    },
    '=': function macro(body) {
