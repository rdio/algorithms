...
var Bujagali = (function() {
  ...
  var module = {
    ...
    fxnLoaded: function(name) {
      var pendingList = pendingExec[name];
      if (pendingList) {
        _.each(pendingList, function(pending) {
          pending.exec();
        });
  ...
  if (root.Backbone) {
    ...
    module.View = Backbone.View.extend({
       ...
       *
       * Calling this will also result in a `rendered` event being triggered
       * and the `onRendered` view method being called.
       *
       * *This function has dangerous side effects.* Since `render` implies
       * replacing every DOM element that is a child of this view's element,
       * all children will be destroyed. This is not a problem the first time
       * the function is called, but care should be take if you call this
       * function more than once as existing child elements and Views will
       * be destroyed.
       *
       * To properly reconstruct a view during re-render, children should be
       * created in templates or in a `beforeRendered` function on the view.
       **/
      render: function(context, callback, parentRendering) {
        var self = this;

        if (self._renderLock) {
          throw new Error("Render called while rendering!");
        }
        self._renderLock = true;

        // Rerendering ourselves implicitly removes our children from the DOM
        // so we destroy them. We don't destroy ourself though, since we're not
        ...
        if (self.createChildComponents) {
          self.createChildComponents();
        }
        ...
        function doneRendering(data) {
          ...
          // If we're the thing being rendered (not a subtree), then we
          // will have been inserted at some point in the callbacks above.
          // Therefore, we can call our onInserted delegate which indicates
        ...
        self.monad.render(context, function(data, markup) {
          $(self.el).html(markup);
          doneRendering = _.after(self.children.length + 1, doneRendering);
          _.each(self.children, function(child) {
      ...
      _addChildFromTemplate: function(containerId, child) {
        if (!child) {
          console.error('tried to add a null component in ' + this.monad.name);
        }
      ...
      /**
       * Bujagali.View#addChild(child) -> Bujagali.View
       * - child (Bujagali.View): The child view to add to this view's
       *   child list.
       *
       * Properly puts the given child into the view's child list. Returns
       * the child.
       **/
      addChild: function(child) {
        ...
        if (this.children) {
          ...
          console.warn('adding child to destroyed component');
        }
        return child;
      },

      /**
       * Bujagali.View#removeChild(child) -> Bujagali.View
       * - child (Bujagali.View): The child view to remove from this
       *   component's child list.
       *
       * This is the correct way to remove a child from a view's child list.
       * This does not destroy the child.
       *
       * Returns the child.
       **/
      ...
      /**
       * Bujagali.View#parent() -> Bujagali.View
       *
      ...
      /**
       * Bujagali.View#remove() -> Bujagali.View
       *
       * Removes this view from the DOM and its parent's children array.
       *
       * Returns this view.
       **/
      remove: function() {
      ...
      /**
       * Bujagali.View#detach() -> Bujagali.View
       *
       * Does not remove the view from the DOM, but calls any
       * `onDetached` delegates to indicate that the View should
       * no longer respond to events. Used for caching a View.
       **/
      detach: function() {
        if (!this._inserted) {
          return;
      ...
      /**
       * Bujagali.View#destroy() -> Bujagali.View
       *
       * Destroys this view. Ensures that all children are also destroyed.
       *
       * Triggers the 'destroy' event, returns this view.
       **/
      destroy: function(parentDying) {
        // parentDying is an internal parameter that optimizes
        // removing an entire tree of children.
        if (!parentDying) {
          this.detach();
          this.remove();
        }
        // We must traverse the tree with detach before we continue destroying.
        // Otherwise, this is what happens:
        // - Parent node gets destroy()
        // - Each child is called with destroy(parentDying = true)
        // - Child skips detach() since parentDying = true
        // - Child sets `this.children` (but not each child) to null
        // - Returns to parent destroy
        // - Calls detach on itself
        // - Each child gets called with detach
        // - Now, each child no longer has any children, so the detach does not propagate to grandchildren
        _.each(this.children, function(child) {
          child.destroy(true);
        });
        this.children = null;
      ...
      /**
       * Bujagali.View#insert() -> Bujagali.View
       *
       * Signals that the View is actually in the DOM and things like
       * measurements will work correctly.
       **/
      insert: function() {
        if (this._inserted) {
          return this;
      ...
      /**
       * Bujagali.View#isRendered() -> boolean
       *
      ...
      /**
       * Bujagali.View#isInserted() -> boolean
       *
       * Returns true if the component is currently inserted.
       **/
      isInserted: function() {
